...
public class GraphToPng {
	...
	private static void CheckForGraphs() {
		...
		if (focusedWindow !=  null && focusedWindow != prev) {
			...
			if (focusedWindow is BaseGraphWindow) {
				// is Shader Graph
	...
	static void GetShaderGraphTypes() {
		...
		materialGraphEditWindowType = typeof(BaseGraphWindow);
		userViewSettingsType = typeof(BaseGraphView);
	}
	...
	static GraphView GetGraphViewFromMaterialGraphEditWindow(EditorWindow win) {
		...
		var x = materialGraphEditWindowType.GetField("graphView", bindingFlags);
		var graphView = (GraphView)x.GetValue(win);
		return graphView;
#if hogehoge
	...
	private static IEnumerator DoShaderGraphToPng() {
		...
		//GraphView graphView = shaderGraphWindow.;
